...
void loginFrame1(int xLenS = 18, int yLenS = 8, int xLenE = 55, int yLenE = 15)
{
    ...
    for(int i=xLenS+1;i<=xLenE-1;i++)         //For Top and Bottom Border line
    {
        ...
        printf("%c",205);  //205 is ASCII value of �
...
void login()
{
	...
	Password[i] = '\0';
    if (strcmp(UserName, "sahil") == 0 && strcmp(Password, "1810") == 0)
    {
...
{
    system("cls");
    gotoXY(35, 10); // Position the loading message
    printf("Loading...");
    gotoXY(30, 12); // Position the loading bar
    for (int i = 0; i < 20; i++)
    {
        Cdelay(100); // Add delay for each character
        printf("%c", 219); // Use a solid block character for better appearance
    }
    gotoXY(55, 12); // Move cursor to end of the loading bar
    printf("\n\n");
}
